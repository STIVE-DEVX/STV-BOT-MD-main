...
const { writeExifImg, writeExifVid } = require('../lib/metaSticker');  
...
async function attpCommand(sock, chatId, message) {  
    ...
    if (!text) {  
        await sock.sendMessage(chatId, { text: 'Veuillez écrire un texte après la commande .attp.' }, { quoted: message });  
        return;  
    }  
  
    try {  
        const mp4Buffer = await renderBlinkingVideoWithFfmpeg(text);  
        const webpPath = await writeExifVid(mp4Buffer, { packname: 'STV BOT MD' });  
        ...
        await sock.sendMessage(chatId, { text: 'Échec lors de la génération du sticker.' }, { quoted: message });  
    }  
}  
